...
- Не хочется ждать возвращения в зону: поставьте поездку на паузу (самокат стоит) и отправьте от
  его имени телеметрию с координатами внутри зоны, затем сразу «Завершить»:
...
| `POST /api/rides` | `{booking_id}` → 201 поездка из своей активной брони (бронь → `used`, самокат → `riding`); повторный старт той же брони — 200 с той же поездкой; 403 чужая бронь, 409 `booking_not_active` / `user_has_active_ride` / `scooter_not_available` |
...
Статус `reserved` телеметрия не меняет, а `riding` — не меняет никогда: во время поездки заряд только
записывается, правило порога применяется один раз при завершении (самокат остаётся `unavailable`,
если разрядился в поездке).
...
- **Правило округления.** Оплата посекундная: `стоимость сегмента = тариф_в_минуту × секунды / 60`,
  округление до копейки половиной вверх — для каждого сегмента отдельно; дробная часть секунды
  отбрасывается. Стоимость езды — сумма сегментов езды, паузы — сумма сегментов паузы, итог — их
