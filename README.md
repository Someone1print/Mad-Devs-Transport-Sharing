...
- Почта — заглушка без SMTP: письма хранятся в базе и видны только своему пользователю
  ...
  проверяется при вводе (без пробелов, ровно один `@`, домен с точкой, не длиннее 254) с
  подсказкой под полем, и ещё раз сервером (422 `invalid_email` с той же подсказкой). Введите
...
- **Подтверждение адреса.** Формат почты проверяется, существование — нет: заглушка не умеет
  отправить код. С настоящей отправкой появится письмо с кодом и флаг «подтверждён».
- **Предупреждение о разрядке заранее.** Панель предупреждает за 5 % до порога; можно добавить
  письмо/уведомление «заряд заканчивается, доедьте до зоны» и запрет старта на самокатах, которым
  заряда не хватит на разумную поездку.
...
- **Вход.** Пользователь идентифицируется заголовком `X-User-Id` и сообщением `identify` в
  WebSocket, оба доверяют клиенту, а «Продолжить как …» на экране имени не спрашивает пароль:
  на одном компьютере любой может открыть чужую поездку, историю и почту. С появлением входа
  (пароль или код на почту — поле для него уже есть) их заменит сессионный токен, а список
  «Продолжить как …» станет списком сохранённых сессий.
