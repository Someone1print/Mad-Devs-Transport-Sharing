...
- Увидеть за пару минут, а не за полчаса: в `.env` задайте ускоренный расход заряда у самоката в
  поездке пользователя и, если хочется, порог повыше (не выше `LOW_BATTERY_THRESHOLD`):

```bash
SIM_USER_RIDE_DRAIN_PER_MINUTE=30   # % в минуту (по умолчанию 0.5)
RIDE_AUTO_FINISH_BATTERY=10
docker compose up -d --build backend simulator
```
...
- Состояние поездки живёт на сервере, вкладка — только окно в него. Начните поездку, закройте
  ...
  завершении покроет всё время, включая проведённое без вкладки.
