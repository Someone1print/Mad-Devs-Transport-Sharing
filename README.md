...
| Чек после завершения внутри зоны | Кабинет, «Поездки»: текущая поездка и история с разбивкой |
| --- | --- |
| ![Чек с разбивкой езды и паузы](docs/images/ride-receipt.jpg) | ![Кабинет с текущей поездкой и историей](docs/images/account.jpg) |

| Почта: по одному письму-чеку на поездку | Почта аккаунта (вход и чеки): подсказка при неверном формате |
| --- | --- |
| ![Почтовый ящик с письмами-чеками](docs/images/mailbox.jpg) | ![Поле почты во вкладке «Аккаунт» с подсказкой под ним](docs/images/email-invalid.jpg) |
...
- Сервер верит только сессии: никакого поля или заголовка с id пользователя в запросах нет, чужой
  id подставить некуда — поездка, история и почта видны только своему владельцу (403/404 на чужие,
  401 без сессии).
...
То же через API, без интерфейса: войдите и возьмите поле `token` из ответа (тот же токен лежит в
cookie браузера), `ID` — номер поездки из `GET /api/rides/active`:

```bash
curl -s -X POST http://localhost:8000/api/auth/login -H "Content-Type: application/json" \
  -d '{"email": "aibek@gmail.com", "password": "ваш пароль"}'          # в ответе — "token": "…"
...
- Почта — заглушка без SMTP: письма хранятся в базе и видны только своему пользователю
  ...
  **Домен** — сервером при сохранении: спрашивает DNS, принимает ли домен почту (MX-запись, без
  ...
  Если `aibek@gmail.con` вдруг сохранился — DNS из контейнера не ответил за 2 с и сработало
  правило «не блокировать пользователя»: в `docker compose logs backend` будет `WARNING … DNS
  check of gmail.con skipped`; повторите с другим адресом (`aibek@gmail2.con`) — в Docker Desktop
  на Windows такое случается на первом запросе после паузы, следующие отвечают за миллисекунды.
...
| 8 | Предварительные требования для развёртывания описаны | сделано | README: «Быстрый старт» (Docker с Compose v2), «Локальная разработка без Docker» (uv, Node 22+) | проходы README из чистого клона — DEVLOG 2026-09-16 и 2026-09-20 |
...
| 13 | Доказуемость: проект доказывает свою работоспособность | сделано | 186 тестов бэкенда (юнит, интеграционные с базой, гонки; 1047 с параметризацией общих векторов), 43 симулятора, 68 фронтенда, общие векторы клиент/сервер в `shared/`; CI `.github/workflows/ci.yml` | зелёный CI на каждом PR, сценарии проверки и скриншоты в README, сквозные прогоны в DEVLOG |
...
| 4 | Ход работы во времени: временные метки от начала до конца | сделано | история коммитов и одиннадцать PR с 8 по 20 сентября 2026; у каждой записи DEVLOG и PROMPTS — дата и время (Asia/Bishkek), записанные в момент решения |
...
Все этапы закрыты, новых функций не планируется. Ниже — что есть на самом деле: что работает,
...
- **Сборка и проверка.** `docker compose up --build` поднимает четыре сервиса; overlay с hot
  reload; CI на каждый PR (ruff, pytest с PostgreSQL, oxlint, vitest, сборка). Тестов: 186 на
  бэкенде (1047 с параметризацией общих векторов), 43 в симуляторе, 68 на фронтенде.
...
17. **Периметр не для публичного запуска.** Нет HTTPS и ограничения частоты запросов; от CSRF
    защищает только `SameSite=Lax` у cookie сессии, отдельного токена нет; учётные данные базы в
    `.env.example` — `scooter`/`scooter`. Для локального демо; перед публикацией нужны обратный
    прокси с TLS и настоящие секреты.
...
4. **Оплата.** Чек должен превращаться в списание. Первый шаг — заглушка платёжной системы, как
   сделана почта: платёж по чеку со статусами «подтверждено», «отклонено» и «зависло» (ответ не
   пришёл — платёж ждёт повторной проверки), оплата счёта идемпотентна (повторная попытка по тому
   же чеку не создаёт второго платежа и не списывает дважды), по итогу — письмо об оплате в ту же
   почту. Дальше — настоящий провайдер за тем же интерфейсом, привязанная карта, запрет новой брони
   при неоплаченном счёте.
...
| `POST /api/auth/register` | `{name, email, password}` → 201 `{user, token}` и cookie `session`; 422 `invalid_email` / `invalid_password` (`problem: too_short` / `too_long`), 409 `email_taken`. Ещё не зарегистрированный аккаунт из старой версии с этой почтой (или, если почты у него нет, с этим именем) присваивается — поездки и история остаются на нём |
| `POST /api/auth/login` | `{email, password}` → 200 `{user, token}` и cookie; 401 `invalid_credentials` (один ответ на неизвестную почту и неверный пароль), 401 `password_not_set` для ещё не зарегистрированного аккаунта из старой версии |
...
заголовок `Authorization: Bearer <token>` с тем же токеном из ответа `/api/auth/login`. Ничего
другого, что называло бы пользователя, сервер не читает. Публичные без сессии: `/api/health*`,
`/api/config`, `/api/scooters`, `/api/telemetry`, `/api/zones`, `/api/auth/*`.
...
- **Аккаунты из старых версий** (строки, у которых ещё нет хеша пароля) не удаляются:
  регистрация с их почтой — или, у строки без почты, с их именем, если оно одно на все такие
  строки, — присваивает строку, и
...
- SMTP нет: «отправить письмо» значит записать строку в таблицу `emails` (получатель, тема, текст,
  `created_at`). Адрес — почта аккаунта; у аккаунта из старой версии, который ещё не
  зарегистрирован и почты не имеет, — транслитерация имени (не длиннее 64 символов, RFC 5321) в
  домене `example.invalid` (зарезервирован RFC 2606, настоящей доставки быть не может). Письма
  отдаёт `GET /api/emails`, только свои; после завершения поездки владелец получает `email.sent`
  по WebSocket.
...
- **Агент и модели.** Код, тесты, миграции и документация написаны агентом Claude Code (Anthropic)
  ...
  телеметрия и реалтайм; бронирование) сделаны моделью Fable 5; этапы 4–10 (поездки и тарификация;
  почта и кабинет; разрядка, восстановление и почта пользователя; финальная полировка; правки
  после прогона; регистрация и вход; документация к сдаче) — Opus 5:
