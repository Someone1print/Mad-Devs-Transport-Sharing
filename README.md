...
и письмо с тем же чеком в почтовый ящик пользователя (почта — заглушка: письма лежат в базе и видны
в кабинете, где есть текущая поездка и история). Всё поднимается одной командой через Docker Compose.
...
| Почта: по одному письму-чеку на поездку |
| --- |
| ![Почтовый ящик с письмами-чеками](docs/images/mailbox.jpg) |
...
- Почта — заглушка без SMTP: адрес строится из имени (`aybek@example.invalid`), письма хранятся
  в базе и видны только своему пользователю (`GET /api/emails`).
...
- **Паритет клиента и сервера — постоянный тест.** `shared/billing-cases.json` (генерируется
  ...
  или геометрии с любой стороны ломает тест, а не расходится молча.
...
│   │   ├── services/         телеметрия, брони, поездки (старт/пауза/завершение/история), почта-заглушка,
│   │   │                     письмо с чеком, фоновый sweeper
...
│   │   ├── account/          почтовый ящик (dedupe, непрочитанные) и история поездок, хуки
│   │   ├── booking/          состояние брони, хук useBooking
│   │   ├── ride/             тарификация в копейках, точка в полигоне, состояние поездки, хуки
│   │   ├── components/       CityMap, ScooterMarkers, ZoneLayer, MyBooking, RidePanel, ReceiptModal, AccountPanel, …
...
- **Настоящая почта.** Письма лежат в таблице `emails` и никуда не уходят. Следующий шаг — фоновый
  отправщик по SMTP (или провайдер вроде SES) поверх той же таблицы: она уже очередь с дедупликацией,
  добавятся `sent_at` и повторы при сбое, а фоновая проверка «завершённая поездка без письма»
  закроет и случай, когда письмо не записалось и никто не повторил `finish`.
- **Поле e-mail в кабинете.** Сейчас адрес выводится из имени, пользователь его не вводил и не
  видел до первого письма. Следующий этап: поле почты в кабинете с проверкой формата на клиенте и
  сервере (один `@`, домен с точкой, без пробелов, разумная длина) и понятной подсказкой при
  ошибке; письма уходят на введённый адрес, транслитерация остаётся запасным вариантом, пока поле
  пустое. Существование адреса без отправки кода не проверить — заглушка ограничится форматом.
- **Страницы в истории и почте.** `GET /api/rides` и `GET /api/emails` отдают всё сразу: для
  демо-парка этого достаточно, для тысяч поездок понадобятся `limit`/курсор и индекс
  `(user_id, finished_at)` на `rides`.
- **История броней.** В кабинете есть история поездок; список прошлых броней (в том числе истёкших
  и отменённых) в интерфейс не выведен, хотя данные в базе есть.
