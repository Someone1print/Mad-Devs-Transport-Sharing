...
1. **Вход без восстановления пароля и без подтверждения почты.** Забытый пароль сбросить нельзя —
   письмо с кодом слать нечем (почта заглушка); аккаунт с опечаткой в почте, прошедшей проверку
   домена, останется с ней. Попыток входа никто не ограничивает: ответы одинаковые, хеш дорогой,
   но перебор не тормозится. Cookie сессии без флага `Secure` — для `http://localhost`;
...
1. **Восстановление пароля и подтверждение почты.** Обе вещи требуют настоящей отправки писем
   (п. 4): письмо со ссылкой на сброс и письмо с кодом при регистрации; плюс ограничение частоты
   попыток входа по адресу и IP — сейчас перебор не тормозится ничем, кроме стоимости argon2.
